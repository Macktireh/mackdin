...
from apps.users.email import (
    send_email,
    send_email_activation_account,
    send_email_activation_account_success,
)
...
def sign_up(request):
    if request.method == "POST":
        ...
        if user_form.is_valid():
            ...
            send_email(
                subject=_("Confirmez l'adresse e-mail de votre compte Mackdin"),
                template_name="users/actiavte.html",
            ...
            messages.error(
                request, _("Merci de bien remplir les informations correctement.")
            )
    else:
    ...
    if request.method == "POST":
        ...
        if user:
            if not user.is_email_verified:
                messages.warning(
                    request, _("Merci de confirmer d'abord votre adresse email.")
                )
                return redirect("sign_in")
            else:
                login(request, user)
        ...
        #     messages.error(request, "ERREUR : mot de passe ou adresse email est incorrect.")
            ...
            messages.error(
                request, _("ERREUR : adresse email ou mot de passe incorrect.")
            )
...
def activate_user(request, uidb64, token):
    ...
    if user and generate_token.check_token(user, token):
        ...
        messages.success(
            request,
            _("Votre email est vérifié avec succès. Vous pouvez vous connecter."),
        )

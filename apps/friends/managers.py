...
class RelationshipManager(models.Manager):
    ...
    def invatation_received(self, receiver):
        ...
        qs = self.import_model().objects.select_related('receiver').filter(receiver=receiver, status='send')
        return qs
    ...
    def invatation_sended(self, sender):
        ...
        qs = self.import_model().objects.select_related('sender').filter(sender=sender, status='send')
        return qs